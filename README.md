...
`.pkg` 로 셋 다 담기며, 세 조각은 실제로 각각 컴파일된 코드다(크기가
422,008 / 418,152 / 446,308 바이트로 서로 다르다). (hppa는 이 기계에
`cc1obj` 백엔드가 없어 제외했다.)

> **설치할 때 남는 아키텍처는 Installer.app 이 정한다.** 설치 화면에
> 아키텍처 목록이 나오고, **고른 것만 남기고 나머지 조각은 떼어 낸다.**
> 기본값대로 두면 그 기계의 아키텍처 하나만 설치된다 — 2026-09-04 에
> i386 실기에서 그렇게 됐고, 설치본이 thin i386(422,008 B)이 되었다.
> 셋을 다 남기려면 **`All Architectures` 를 고른다.** 패키지 쪽 문제가
> 아니다(페이로드는 그대로 fat 이다).
